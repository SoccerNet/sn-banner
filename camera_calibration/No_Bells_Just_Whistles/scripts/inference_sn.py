...
def parse_args():
    ...
    parser.add_argument(
        "--use_gt",
        action="store_true",
        help="Use ground truth annotations (default: False)",
    )
...
if __name__ == "__main__":
    ...
    if args.main_cam_only:
        cam_info = json.load(
            open(args.root_dir + args.split + "/match_info_cam_gt.json")
        )
        files = [file for file in files if file.split("/")[-1] in cam_info.keys()]
        files = [
            file
            for file in files
            if cam_info[file.split("/")[-1]]["camera"] == "Main camera center"
        ]
        ...
        with zipfile.ZipFile(zip_name_pred, "w") as zip_file:
            ...
            for file in tqdm(files, desc="Processing Images"):
                ...
                final_dict = complete_keypoints(
                    kp_dict, lines_dict, w=image.width, h=image.height, normalize=True
                )
